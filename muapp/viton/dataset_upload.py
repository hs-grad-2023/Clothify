...
2. 장고 쉘에서 다음과 같이 코드를 입력하여 데이터를 업로드합니다.
    ...
    exec(open('dataset_upload.py', encoding='utf-8').read())
...
viton_upload_model.objects.filter(id__gte=500).delete()
viton_upload_cloth.objects.filter(id__gte=500).delete()
...
for root, dirs, files in os.walk(result_dir):
    for result in files:
        ...
        # 모델 인스턴스 생성 및 저장
        result = viton_upload_result(name=result, image=file)
        result.save()
